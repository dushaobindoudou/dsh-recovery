...
import { recordKnownGood, latestKnownGood, readSnapshotManifest } from './knowngood.js';
...
export async function repair(checks, env, opts = {}) {
  ...
  // A repair run that ends healthy is the moment this installation is known to
  // work, so it is the moment worth recording — and the only one, since
  // `detect` may not write. Every surface reaches repair through here, so the
  // CLI, the slash command and the settings page all record the same way.
  const recorded = after.healthy ? recordKnownGood(env) : null;
  return { applied, healthyAfter: after.healthy, results: after.results, recorded };
}
...
export function renderReport({ results, healthy }, env) {
  ...
  lines.push('');
  lines.push(healthy ? 'No problems found.' : 'Run `/doctor fix` (or `dsh-recovery fix`) to apply the fixable repairs.');
  // What is left after the fixable repairs is, for three of the five checks,
  // something only the user can decide — unless a healthy configuration was
  // recorded, in which case putting it back is the answer.
  if (!healthy && env?.profileRoot !== undefined) {
    const snapshot = latestKnownGood(env.profileRoot);
    if (snapshot !== null) {
      const at = readSnapshotManifest(snapshot)?.recordedAt;
      lines.push(`A known-good configuration${at === undefined ? '' : ` from ${at}`} is on record — \`dsh doctor rollback\` puts it back.`);
    }
  }
  return lines.join('\n');
}
