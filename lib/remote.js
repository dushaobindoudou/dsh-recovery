...
import { diagnose, repair, rollback } from './doctor.js';
import { latestKnownGood, readSnapshotManifest, rollbackToKnownGood } from './knowngood.js';
...
/** The reader's language for the AI analysis; anything unknown means Chinese. */
function localeOf(request) {
  const raw = request === null || typeof request !== 'object' ? undefined : request.locale;
  return raw === 'en' ? 'en' : 'zh';
}

/**
 * What the page needs to label its rollback button: when the last known-good
 * configuration was recorded, or null when none is on record.
 */
function knownGoodOf(env) {
  const dir = latestKnownGood(env.profileRoot);
...
export function aiCapability(ctx) {
  ...
  try {
    if (llm !== undefined && typeof llm.listProviders === 'function') {
      ...
      providerKnown = Array.isArray(providers) && providers.some((p) => p !== null && typeof p === 'object' && (p.id === provider || p.provider === provider));
    }
  } catch {
    providerKnown = false;
  }
 ...
 */
export async function aiDiagnose(ctx, env, results, locale = 'zh') {
  ...
  // The analysis is read on the settings page, so it has to come back in the
  // language that page is being read in.
  const prompt = (locale === 'en' ? [
    'You are an installation-diagnostics assistant for dsh (DeepSeek Harness). Below is the JSON snapshot',
    'collected by one automated rule check. Judge only from the evidence in the snapshot: 1) which real',
    'problems exist (list only what the snapshot proves — never speculate); 2) the cause of each and the',
    'repair to make; 3) if everything is fine, say so plainly. Do not invent anything outside the snapshot,',
    'and do not suggest changing anything the snapshot does not show. Answer in English, as concise bullets.',
  ] : [
  ...
  ]).concat(['', JSON.stringify(snapshot, null, 2)]).join('\n');
  ...
  try {
    const options = {
      ...
      messages: [{
        ...
        source: { kind: 'plugin', plugin: 'dsh-recovery' },
      }],
    };
...
export class DoctorRemote extends TypertRemoteService {
  ...
  async status(request) {
    ...
    const base = {
      ...
      knownGood: knownGoodOf(resolved.env),
    };
    if (mode !== 'ai') return base;
    const ai = await aiDiagnose(this.ctx, resolved.env, results, localeOf(request));
    return { ...base, ai };
  }
  ...
  /**
   * Reverse this tool's most recent fix, then re-diagnose.
   *
   * The page could fix but never undo, while "every fix is reversible" is the
   * tool's premise — an escape hatch reachable only from a terminal is not one
   * for someone who lives in the settings page.
   */
  async restore(request) {
    const resolved = resolveEnv();
    if (resolved.error !== undefined) return { error: resolved.error };
    const only = onlyOf(request);
    const outcome = await rollback(checks, resolved.env, only === undefined ? {} : { only });
    return { env: resolved.env, ...outcome, knownGood: knownGoodOf(resolved.env) };
  }

  /**
   * Put the last known-good configuration back, then re-diagnose.
   *
   * This is the answer for the report-only checks: the tool cannot know what a
   * damaged config was meant to say, but it knows what it used to be.
   */
  async rollback(request) {
    const resolved = resolveEnv();
    if (resolved.error !== undefined) return { error: resolved.error };
    const rolledBack = rollbackToKnownGood(resolved.env);
    const { results, healthy } = await diagnose(checks, resolved.env);
    return { env: resolved.env, rolledBack, results, healthy, knownGood: knownGoodOf(resolved.env) };
  }
  ...
  async info(request) {
    ...
    return {
      ...
      ai: aiCapability(this.ctx),
      knownGood: resolved.error === undefined ? knownGoodOf(resolved.env) : null,
    };
  }
}
...
export function installDoctorRemote(ctx) {
  ...
  for (const m of ['status', 'fix', 'restore', 'rollback', 'info']) markRemoteMethod(DoctorRemote.prototype, m);
